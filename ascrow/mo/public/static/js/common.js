		...
		if(mCheck()){
			...
			if( $this.is('.tab_cate a')){                
				...
				if(!$this.closest('li').hasClass('on')){
                    $('#gnb_m').removeClass('hide');
                    $('#gnb_m').css('height','100%');
                    $('body').css({'overflow' : 'hidden'});
                    $('.bar_tabnav').hide();
...
$(document).ready(function(){
	...
	//필터 탭
	$('.filter_tab_wrap li a').each(function(){
		$(this).click(function(){
			$(this).addClass('_selected').parent().siblings().children().removeClass('_selected');
			$('.filter_content_wrap > div').eq().removeClass('hide').siblings().addClass('hide');
			return false;
		});
	});
	...
	$(".button_filter_reset").click(function() {
		//alert('test');
		$('.filter_tab_wrap > li a').removeClass('._selected');
		$('filter_tab_wrap > li:first-child a').addClass('._selected');
		$('.filter_content input[type=checkbox]').prop('checked',false); 
	});
});